...
class RegisterController
{
    public function index(): void
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $cust_first_name = trim($_POST['first_name'] ?? '');
            $cust_last_name  = trim($_POST['last_name'] ?? '');
            $cust_phoneNo      = trim($_POST['phone'] ?? '');
            $email      = trim($_POST['email'] ?? '');
            $password   = $_POST['password'] ?? '';

            // ---- Validation ----
            if ($cust_first_name === '') $errors[] = "First name is required.";
            if ($cust_last_name === '')  $errors[] = "Last name is required.";
            ...
            password
                ...
                if (!$stmt) {
                    ...
                    $stmt->bind_param("sssss", $cust_first_name, $cust_last_name, $cust_phoneNo, $email, $hashed_password);
